...
def synthesize_verilog(folder_path, project_name):
    ...
    for vfile in verilog_files:
        ...
        if result.returncode != 0:
            st.error(f"Error in synthesizing {vfile}: {result.stderr}")
...
# Streamlit UI
st.title("Verilog Code Synthesis")

project_name = st.selectbox("Select Project", get_project_list())
folder_path = st.text_input("Enter Folder Path")

if st.button("Synthesize"):
    if project_name and folder_path:
        if os.path.exists(folder_path):
            output_folder = synthesize_verilog(folder_path, project_name)
            if output_folder:
                st.success("Synthesis Completed! Images saved in synthesized_image folder.")
                display_images(output_folder)
            else:
                st.error("No Verilog files found for synthesis!")
        else:
            st.error("Invalid folder path!")
    else:
        st.error("Please select a project and enter a valid folder path.")